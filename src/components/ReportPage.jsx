...
export default function ReportPage({ reportId, theme: propTheme }) {
  ...
  const primaryColor = theme?.primary_color || '#37386E';   // your academy's default
    ...
    <div className="p-4 md:p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={handleDownloadPdf} disabled={!rows.length} className="bg-primary hover:bg-accent text-white px-4 py-2 rounded-lg text-sm flex items-center gap-2 disabled:opacity-50">
      ...
      {isError && <div className="bg-red-50 text-red-700 rounded-lg p-4">Failed to load report: {error?.message || 'Unknown error'}</div>}
        ...
        <div className="bg-white rounded-xl border shadow-sm p-4">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-center">
            ...
            <div>
              ...
              <p className="text-2xl font-bold" style={{ color: primaryColor }}>
                {rows.filter(r => r.status === 'Admitted').length}
              </p>
            </div>
            <div>
            ...
            <thead className="bg-primary-bg text-primary-dark">
              <tr>
                {config.columns.map(col => (
                  <th key={col.accessor} className="p-3 text-left font-medium whitespace-nowrap">{col.header}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {rows.map((row, rowIdx) => (
                <tr key={rowIdx} className="border-t hover:bg-gray-50 transition-colors">
                  {config.columns.map(col => (
                    <td key={col.accessor} className="p-3 whitespace-nowrap text-secondary-dark">{resolvePath(row, col.accessor) ?? '—'}</td>
                  ))}
                </tr>
              ))}
            </tbody>
              ...
              <tfoot className="bg-gray-100 font-semibold text-primary-dark">
                <tr>
                  {config.columns.map((col, idx) => (
                    <td key={col.accessor} className="p-3 whitespace-nowrap">{aggregateRowObj[col.accessor] ?? ''}</td>
                  ))}
                </tr>
              </tfoot>
            )}
